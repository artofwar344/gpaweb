...
$(function() {
	var viewTarget = function(eid) {
		$.post("/reportmeetingcomment/gettarget", { "eid": eid }, function(targetid) {
			window.open("{{ 'http://share.' . app()->env . '/meeting/detail?id=' }}" + targetid, '_blank');
		});
		return false;
	};

	var disableTarget = function(eid) {
		$.post("/reportmeetingcomment/disabletarget", { "eid": eid }, function(ret) {
			backend.list();
		});
		return false;
	};
	...
	var rejectReport = function(eid) {
		$.post("/reportmeetingcomment/rejectreport", { "eid": eid }, function(ret) {
			backend.list();
		});
		return false;
	};
	...
	var backend = $.backend({
		...
		category: "举报状态",
		operators: [
			{ type: "callback", callback: rejectReport, enable: dealEnable, tip: "将该评论的举报驳回", text: "驳回举报", css: "btn_auth" },
			{ type: "callback", callback: disableTarget, enable: dealEnable, tip: "将该评论设为禁用状态", text: "禁用评论", css: "btn_delete" },
	...
	$(".multi_actions .button_auth").click(function() {
		...
		$.post("/reportmeetingcomment/rejectreportmulti", { eids: backend.checkedRow() }, function() {
	...
	$(".multi_actions .button_disagree").click(function() {
		if ($(this).hasClass("button_1_disabled")) return false;
		$(this).addClass("button_1_disabled");
		$.post("/reportmeetingcomment/disabletargetmulti", { eids: backend.checkedRow() }, function() {
			backend.clearCheckedRow();
			backend.list();
		});
		return false;
...
<div class="multi_actions">
	...
	<a href="#" class="button_1 button_1_disabled button_disagree">禁用评论</a>
</div>
