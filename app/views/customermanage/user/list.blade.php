...
var departmentId = {{ $manager_department_id }};
var keyassignRole @if (Ca\Service\ManagerService::check_role('keyassign')) = true @endif;
var keyusageRole @if (Ca\Service\ManagerService::check_role('keyusage')) = true @endif;
$(function() {
	...
	$(".main_actions .button_import").click(function() {
		...
		formImportReset();
		$(".info", dlgImport).html("选择用户列表文件批量导入用户!<p class='more_info'>[文件格式: <strong>csv</strong>, 文件编码: <strong>gbk</strong>] (<a href=\"{{ Config::get('app.asset_url') }}files/import_sample.csv\">样本</a>)</p>");
		$(".error_list", dlgImport).html("");
		return false;
	});
...
@if (Ca\Service\ManagerService::check_role('user.new') == false || Input::get('inner'))
@actions (array('title' => (empty($department_name) ? '用户管理' : '部门: ' . $department_name), 'action' => '用户', 'buttons' => array()))
@else
@if (Auth::user()->top)
@actions (array('title' => (empty($department_name) ? '用户管理' : '部门: ' . $department_name), 'action' => '用户', 'buttons' => array('add')))
@else
@actions (array('title' => (empty($department_name) ? '用户管理' : '部门: ' . $department_name), 'action' => '用户', 'buttons' => array('add', 'import')))
@endif
@endif
