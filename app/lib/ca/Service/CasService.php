...
EOT;
	...
	public static function validateOut($params = "")
	{
		if(app()->env == 'ms.eurasia.edu') {
			...
			$phpCas->setServerLoginUrl("http://ms.eurasia.edu"); 
			$phpCas->forceAuthentication();
			$phpCas->logout($params);
		}else{
			if(!app()->env == 'gtcfla.gpa.edu.cn')
				include_once(app_path() . "/lib/CAS.php");
			else
				include_once(app_path() . "/lib/gtcfla/CAS.php");

			$phpCas = new \phpCAS();
			if (app()->env == 'wxit.gpa.edu.cn')
			{
				$phpCas->setDebug();
			}
			...
			$phpCas->setDebug('./CASlog.log');
			if(app()->env == 'gtcfla.gpa.edu.cn')
				$info = parse_url(ParamsService::get('casloginurl1'));
			else
				$info = parse_url(ParamsService::get('casloginurl'));

			$info['uri']  = array_get(explode('/', $info['path']), 1);
			$info['host'] = array_get(explode('/', $info['path']), 0);
			//var_dump($info);
			if(app()->env == 'gtcfla.gpa.edu.cn')
				$phpCas->client(CAS_VERSION_2_0,$info['host'],(int)ParamsService::get('casport1'),$info['uri'],false);
			else
				$phpCas->client(CAS_VERSION_2_0,$info['host'],(int)ParamsService::get('casport'),$info['uri'],false);
	...
	public static function validateTicket(&$name)
	{
		if(app()->env == 'ms.eurasia.edu') {
			...
			$phpCas = new \phpCAS();
			$phpCas->setDebug('./CASlog.log');
			if(app()->env == 'gtcfla.gpa.edu.cn' || app()->env == 'ms.eurasia.edu')
				$info = parse_url(ParamsService::get('casloginurl'));
			else	
				$info = parse_url(ParamsService::get('casloginurl'));
			...
			//echo 1;exit;
			//var_dump($info);exit;
			//$phpCas->client(CAS_VERSION_2_0,'cas.gtcfla.edu.cn',443,'lyuapServer',false);
			if(app()->env == 'gtcfla.gpa.edu.cn')
			{
				$phpCas->client(CAS_VERSION_2_0,'cas.gtcfla.edu.cn',443,'lyuapServer',false);
				//$phpCas->client(CAS_VERSION_2_0, $info['host'], (int)ParamsService::get('casport'), $info['uri'], false);
			...
			//$phpCas->setServerLoginUrl("http://user.gtcfla.gpa.edu.cn/login");
			// 验证用户
			$phpCas->setNoCasServerValidation();
			if(!app()->env == 'gtcfla.gpa.edu.cn') 
			{
